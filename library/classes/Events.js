import { ItemStack,Block, world } from '@minecraft/server';
...
/**
 * @typedef BlockActivate
 * @property {Block} block Block that  was activated/powered on
 */
/**
...
class ItemVirtualizedBeforeEvent {
    ...
    constructor(types) {
        ...
        create.broadcastEvent('itemVirtualizedBeforeEvent',false,JSON.stringify(this.canceled))
...
class MechanicalMixerProcessBeforeEvent {
    ...
    constructor(types) {
        ...
        wait(this.canceled===true)
        create.broadcastEvent('mechanicalMixerProcess',false,JSON.stringify(this.canceled))
    }
}
...
/**
 * Represents a signal for the CreateBlockActivateAfter event in Create.
 * @class
 */
class CreateBlockActivateAfterEventSignal {
    /**
     * Creates an instance of CreateBlockActivateAfterEvent Signal.
     */
    constructor() {
        /**
         * @private
    ...
    /**
     * Subscribes to the CreateBlockActivateAfter event and provides a callback.
     * @param {function(CreateBlockActivateAfterEvent): void} callback - The callback function to be executed when the event is triggered.
     * @returns {string} A subscriber ID for unsubscribing later.
     * @example
     * const listener = create.afterEvents.blockActivate.subscribe((event) => {
     *     // Handle the event here
     * });
     * create.afterEvents.blockActivate.unsubscribe(listener);
     */
    subscribe(callback = (arg) => {
        const subscriberId = generateId();
        this.subscribers[subscriberId] = callback;
        ...
        const eventCallback = (ev) => {
            const { id, sourceBlock } = ev;
                if (id === 'create:blockActivateAfterEvent') {
                // Create an instance of MechanicalMixerProcessAfterEvent and use it.
                const eventData = new CreateBlockActivateAfterEvent({block:sourceBlock});
                if (subscriberId in this.subscribers) {
                    this.subscribers[subscriberId](eventData);
                }
    ...
    /**
     * Unsubscribes a subscriber from the CreateBlockActivateAfterEvent event.
     * @param {string} subscriberId - The ID of the subscriber to unsubscribe.
     */
    unsubscribe(subscriberId) {
        delete this.subscribers[subscriberId];
...
class CreateBlockActivateAfterEvent {
    /**
     * @param {BlockActivate} types 
     */
    constructor(types) {
        /**
         * Block that was activated/powered on
         */
...
class Create {
     ...
     * @param {boolean} wait
     */
    broadcastEvent(eventName,wait = false,data){
     ...
     * 
     * @param {string} eventName 
     * @param {function(typeof arg)} callback 
     */
    waitFor(eventName,callback = (arg))
}
...
class CreateAfterEvents {
    constructor() {
        ...
        this.blockActivate = new CreateBlockActivateAfterEventSignal()
    }
}
...
create.afterEvents.blockActivate.subscribe(ev=>{
    const {block} = ev
    block.dimension
})
